...
def my_model(features, labels, mode, params):
    ...
    #lr
    regularizer = tf.contrib.layers.l1_regularizer(params['regularizer'])
    with tf.variable_scope('input_layer'):
        ...
        fm_list = []
        for k, v in features.items():
            embedding_table = tf.get_variable(k + '_embed_fm', [params['feature_columns'][k], params['dim']], regularizer=regularizer)
            embedded_var = tf.nn.embedding_lookup(embedding_table, v)
    ...
    print(bi_net)
    bi_net = tf.layers.dense(bi_net, units=params['dim'], activation=tf.nn.relu,
                             kernel_initializer=tf.truncated_normal_initializer(stddev=tf.sqrt(2/66)),
                             kernel_regularizer=regularizer, bias_regularizer=regularizer)
    bi_net = tf.layers.dense(bi_net, units=params['dim'], activation=tf.nn.relu,
                             kernel_initializer=tf.truncated_normal_initializer(stddev=tf.sqrt(2/66)),
                             kernel_regularizer=regularizer, bias_regularizer=regularizer)
    bi_net = tf.layers.dense(bi_net, units=params['dim'], activation=None,
                             kernel_initializer=tf.truncated_normal_initializer(stddev=tf.sqrt(2/66)),
                             kernel_regularizer=regularizer, bias_regularizer=regularizer)
    ...
    exp_rate = tf.train.exponential_decay(learning_rate=params['learning_rate'], global_step=tf.train.get_global_step(),
                                          decay_rate=0.9, decay_steps=10000)
...
def main(_):
    ...
    if False:
        ws = tf.estimator.WarmStartSettings(
            ckpt_to_initialize_from="/home/lucius/Projects/notebook/homework/ctr_predict/models/v1/logdata_fm_10",
    ...
    for i in range(5):
        ...
        for j in range(5):
            model_reg = (10**(-j)) * 1.0
            model_dir = os.path.join(os.path.dirname(__file__), 'logdata_' + str(i) + '_' + str(j))
            for k in range(2):
                classifier = tf.estimator.Estimator(
                    model_fn=my_model,
                    params={
                        'feature_columns': build_model_columns_fm(),
                        'optimizer': 'sgd' if k != 0 else None,
                        'learning_rate': 0.001,
                        'dim': fm_dim,
                        'regularizer': model_reg
                    },
                    model_dir=model_dir,
                    warm_start_from=ws)
