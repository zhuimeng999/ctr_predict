...
def my_model(features, labels, mode, params):
    ...
    with tf.variable_scope('input_layer'):
        ...
        fm_list = []
        for k, v in features.items():
            embedding_table = tf.get_variable(k + '_embed_fm', [params['feature_columns'][k], 3], regularizer=regularizer)
            embedded_var = tf.nn.embedding_lookup(embedding_table, v)
    ...
    net = tf.layers.dense(bi_net, units=3, activation=tf.nn.relu,
                             kernel_initializer=tf.truncated_normal_initializer(stddev=tf.sqrt(2/66)),
                             kernel_regularizer=regularizer, bias_regularizer=regularizer)
    net = tf.layers.dense(net, units=3, activation=tf.nn.relu,
                             kernel_initializer=tf.truncated_normal_initializer(stddev=tf.sqrt(2/66)),
                             kernel_regularizer=regularizer, bias_regularizer=regularizer)
...
def main(_):
    ...
    if True:
        ws = tf.estimator.WarmStartSettings(
            ckpt_to_initialize_from="/home/lucius/Projects/notebook/homework/ctr_predict/models/v2/logdata_4_4",
