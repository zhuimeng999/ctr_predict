...
from models.v1.v1_dataset import build_model_columns, get_input_fn
...
def my_model(features, labels, mode, params):
    ...
    logits = tf.reduce_sum(net, axis=1) + lr_bias
    # Compute predictions.
...
def main(_):
    model_dir = os.path.join(os.path.dirname(__file__), 'logdata_lr')
    print(model_dir)
    ...
    for i in range(15):
        ...
        # Train the Model.
        classifier.train(
            input_fn=get_input_fn(os.path.join(os.path.dirname(__file__), '../../FE/FE1/train_split.csv'), 256, 1, 5000))

        # Evaluate the model.
        eval_result = classifier.evaluate(
            input_fn=get_input_fn(os.path.join(os.path.dirname(__file__), '../../FE/FE1/valid_split.csv'), 256, 1, 5000))
