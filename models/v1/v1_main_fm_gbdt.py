...
from models.v1.v1_dataset_gbdt import build_model_columns_fm, get_input_fn, COLUMN_NAMES
...
def my_model(features, labels, mode, params):
    ...
    # regularizer = tf.contrib.layers.l1_regularizer(0.00001)
    regularizer1 = tf.contrib.layers.l1_regularizer(0.0001)
    regularizer = None
    with tf.variable_scope('input_layer'):
        ...
        for k, v in features.items():
            if k in fm_exlude_features:
                ...
                embedding_table = tf.get_variable(k + '_embed_lr', [params['feature_columns'][k], 1], regularizer=regularizer1)
            else:
...
def main(_):
    ...
    for i in range(15):
        ...
        # Train the Model.
        classifier.train(
          input_fn=get_input_fn(os.path.join(os.path.dirname(__file__), '../../FE/FE3/train_gbdt.csv'), 512, 1, 5000),
            steps=None)

        # Evaluate the model.
        eval_result = classifier.evaluate(
            input_fn=get_input_fn(os.path.join(os.path.dirname(__file__), '../../FE/FE3/valid_gbdt.csv'), 1024, 1, 5000),
            steps=None)
