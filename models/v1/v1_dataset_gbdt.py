...
for i in range(100):
    COLUMN_NAMES.append('gbdt'+str(i))
    _COLUMN_DIM.append(127)
...
def parse_csv(value):
    columns = tf.decode_csv(value, record_defaults=_CSV_COLUMN_DEFAULTS[1:], select_cols=list(range(len(_CSV_COLUMN_DEFAULTS)))[1:])
...
def get_input_fn(train_path, batch_size, repeat, shuffle, use_tfrecord=False):
    def csv_input_fn():
        ...
        # dataset = tf.contrib.data.CsvDataset(train_path, _CSV_COLUMN_DEFAULTS)
    ...
    #     #dataset = dataset.batch(batch_size)
    #     #dataset = dataset.map(parse_csv, num_parallel_calls=None)
    #     dataset = dataset.apply(tf.contrib.data.map_and_batch(parse_csv, batch_size, num_parallel_calls=8))
    #     dataset = dataset.apply(tf.contrib.data.unbatch())
    #     print(dataset)
    #     return dataset
    # def csv_input_fn():
    #     return tf.contrib.data.make_csv_dataset(train_path, batch_size, column_names=COLUMN_NAMES,
    #                                             column_defaults=_CSV_COLUMN_DEFAULTS[1:],
    #                                             label_name='click',
    #                                             select_columns=COLUMN_NAMES[1:],
    #                                             sloppy=True,
    #                                             num_parallel_parser_calls=8)
    # def csv_input_fn():
    #     ds = tf.data.Dataset.from_generator(gen_dataset, tf.int32)
